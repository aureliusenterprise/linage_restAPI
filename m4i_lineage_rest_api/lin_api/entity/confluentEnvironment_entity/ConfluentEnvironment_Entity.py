...
class confluentEnvironment_Class(Resource):
    ...
    @api.doc(id='get_confluentEnvironment_entities')
    @api.marshal_with(m4i_output_get_model)
    def get(self):
        """
        Returns list of confluentEnvironment Entities
        """
        search_result = asyncio.run(get_entities_by_type_name("m4i_confluent_environment"))
    ...
    @api.doc(id='post_confluentEnvironment_entities')
