...
class generic_process_Class(Resource):
    ...
    @api.doc(id='get_generic_process_entities')
    @api.marshal_with(m4i_output_get_model)
    def get(self):
        """
        Returns list of Generic Process Entities
        """
        search_result = asyncio.run(get_entities_by_type_name("m4i_generic_process"))
    ...
    @api.doc(id='post_generic_process_entities')
